...
$LogPath = Join-Path $LogDir "PA-A21-R2-CONTENT-WRITE-$Timestamp.log"
...
function Invoke-GitHubProbe {
    ...
    $headers = @{
        ...
        'User-Agent' = 'pa-safra-a21-r2-gate'
    }
    ...
    try {
        ...
        if ($_.Exception.Response) {
            ...
            try { $accepted = [string]$_.Exception.Response.Headers['X-Accepted-GitHub-Permissions'] } catch {}
            try {
...
function Assert-GitHubTokenAccess {
    param([Parameter(Mandatory = $true)][string]$Token)

    $identity = Invoke-GitHubProbe -Uri 'https://api.github.com/user' -Token $Token
    if ($identity.Status -ne 200) {
        if ($identity.Status -eq 401) {
            throw 'TOKEN GITHUB REJEITADO (HTTP 401). TOKEN INVALIDO, EXPIRADO, REVOGADO OU COPIADO INCORRETAMENTE. NENHUM SECRET FOI ALTERADO.'
        }
        if ($identity.Status -eq 403) {
            throw 'TOKEN GITHUB RECUSADO (HTTP 403) NA VALIDACAO DE IDENTIDADE. VERIFIQUE RESTRICOES/APPROVAL DO TOKEN. NENHUM SECRET FOI ALTERADO.'
        }
        throw "TOKEN GITHUB NAO PODE SER VALIDADO. HTTP=$($identity.Status). NENHUM SECRET FOI ALTERADO."
    }
    Write-Log 'Token GitHub autenticado pela API (identidade confirmada; valor nao registrado).'

    $encodedRef = [uri]::EscapeDataString($ContentBranch)
    $probe = Invoke-GitHubProbe -Uri "$RepositorioApi/contents/public/content/videos.json?ref=$encodedRef" -Token $Token
    if ($probe.Status -ne 200) {
        if (-not [string]::IsNullOrWhiteSpace($probe.AcceptedPermissions)) {
            Write-Log "GitHub informou permissoes aceitas pelo endpoint: $($probe.AcceptedPermissions)"
        }
        if ($probe.Status -eq 403) {
            throw 'TOKEN VALIDO, MAS O GITHUB NEGOU O CONTEUDO (HTTP 403). CONFIRA RESOURCE OWNER, REPOSITORIO SELECIONADO E CONTENTS: READ AND WRITE. NENHUM SECRET FOI ALTERADO.'
        }
        if ($probe.Status -eq 404) {
            throw 'TOKEN VALIDO, MAS O REPOSITORIO/BRANCH NAO ESTA VISIVEL PARA ELE (HTTP 404). CONFIRA RESOURCE OWNER E ONLY SELECT REPOSITORIES. NENHUM SECRET FOI ALTERADO.'
        }
        throw "TOKEN VALIDO, MAS A LEITURA DA BRANCH EDITORIAL FALHOU. HTTP=$($probe.Status). NENHUM SECRET FOI ALTERADO."
    }

    try {
        $file = $probe.Body | ConvertFrom-Json
    }
    catch {
        throw 'GITHUB RESPONDEU 200, MAS O CONTEUDO DE videos.json NAO PODE SER INTERPRETADO.'
    }
    if ([string]::IsNullOrWhiteSpace([string]$file.sha) -or [string]::IsNullOrWhiteSpace([string]$file.content)) {
        throw 'GITHUB RESPONDEU 200, MAS NAO RETORNOU SHA/CONTEUDO ESPERADOS.'
    }

    Write-Log "Token GitHub confirmou acesso a content/pa-v001-admin-preview; SHA videos=$(([string]$file.sha).Substring(0,12))."
}
...
try {
    Write-Host ''
    Write-Host 'PA SAFRA - A21 R2 - CONTINUACAO DO TOKEN E ESCRITA EDITORIAL' -ForegroundColor Cyan
    Write-Host 'O build formal de 08:56 foi preservado; esta rotina nao o repete.' -ForegroundColor Cyan
    Write-Host ''
    ...
    foreach ($name in $RootConfigNames) {
        if (Test-Path -LiteralPath (Join-Path $Raiz $name) -PathType Leaf) {
            throw "CONFIG WRANGLER NO ROOT DETECTADA ($name). EXECUCAO BLOQUEADA."
        }
    }
    ...
    if ($contentHead -ne $AppCheckpoint) {
        throw "BRANCH EDITORIAL SAIU DO CHECKPOINT SEGURO. ESPERADO=$AppCheckpoint OBTIDO=$contentHead"
    }
    ...
    $changedRaw = (Invoke-Native -Command 'git.exe' -Arguments @('diff','--name-only',"$AppCheckpoint..$head") -Label 'Auditar delta R2').Text
    $changed = @()
    ...
    $unsafe = @($changed | Where-Object {
        ...
        $_ -notlike 'tools/PA-SAFRA-A21-R2-*'
    })
    if ($unsafe.Count -gt 0) {
        throw "R2 CONTEM ALTERACAO FORA DO ESCOPO PROCEDURAL: $($unsafe -join ', ')"
    }
    Write-Log "A21 R2 sincronizada no HEAD $head; delta desde o app checkpoint e somente procedural."

    if (-not (Test-Path -LiteralPath (Join-Path $Raiz 'dist') -PathType Container)) {
        throw 'DIST DO BUILD FORMAL ANTERIOR NAO ESTA DISPONIVEL. NAO VOU RECONSTRUIR AUTOMATICAMENTE.'
    }
    Invoke-Native -Command 'npm.cmd' -Arguments @('run','smoke') -Label 'Revalidar dist existente' | Out-Null
    Write-Log 'Dist do build formal anterior reutilizado e smoke revalidado; build nao repetido.'
    ...
    Write-Log 'Wrangler e D1 de Preview confirmados.'

    Write-Host ''
    Write-Host 'Cole o Fine-grained Personal Access Token do GitHub.' -ForegroundColor Cyan
    Write-Host 'O valor nao sera exibido nem registrado.' -ForegroundColor Cyan
    ...
    if ([string]::IsNullOrWhiteSpace($Pat) -or $Pat.Length -lt 20) {
        throw 'TOKEN GITHUB AUSENTE OU CURTO DEMAIS. NENHUM SECRET FOI ALTERADO.'
    }

    Assert-GitHubTokenAccess -Token $Pat
    ...
    Write-Log 'GITHUB_CONTENT_TOKEN confirmado somente no Preview.'

    $deploy = Invoke-Wrangler -Arguments @(
        'pages','deploy','dist',
        '--project-name',$ProjetoPages,
        '--branch',$ContentBranch,
        '--commit-hash',$AppCheckpoint,
        '--commit-message','PA-V001-A21 R2 preview content write e2e',
        '--experimental-provision=false',
        '--experimental-auto-create=false',
        '--install-skills=false'
    ) -Label 'Deploy Preview A21 R2'

    $urlMatches = [regex]::Matches($deploy.Text, 'https://[A-Za-z0-9.-]+\.pages\.dev')
    if ($urlMatches.Count -lt 1) {
        throw 'DEPLOY A21 R2 CONCLUIU, MAS A URL PAGES.DEV NAO FOI IDENTIFICADA.'
    }
    $PreviewUrl = $urlMatches[$urlMatches.Count - 1].Value.TrimEnd('/')
    Write-Log "Preview A21 R2: $PreviewUrl"
    ...
    if (-not $status0.enabled -or -not $status0.credentials_configured -or -not $status0.rate_limit_configured -or $status0.rate_limit_backend -ne 'd1') {
        throw 'PREVIEW A21 R2 NAO CONFIRMOU AUTH + D1.'
    }
    if (-not $status0.token_configured -or $status0.write_enabled) {
        throw 'ESTADO DE ESCRITA ANTES DO LOGIN E INESPERADO.'
    }
    if ([string]$status0.branch -ne $ContentBranch) {
        throw "BRANCH EDITORIAL INESPERADA NO RUNTIME: $($status0.branch)"
    }
    Write-Log 'Status sem sessao: token presente, D1 presente, branch correta e write_enabled=false.'
    ...
    if (-not $login.ok) {
        throw 'LOGIN A21 R2 NAO RETORNOU ok=true.'
    }
    ...
    if (-not $status1.authenticated -or -not $status1.token_configured -or -not $status1.write_enabled) {
        throw 'SESSAO A21 R2 NAO HABILITOU ESCRITA CONTROLADA.'
    }
    if ([string]$status1.branch -ne $ContentBranch) {
        throw 'SESSAO A21 R2 APONTOU PARA BRANCH EDITORIAL INESPERADA.'
    }
    ...
    $testMarker = "PA-SAFRA-A21-R2-$([guid]::NewGuid().ToString('N'))"
    $nextVideos = @($OriginalVideos) + @([pscustomobject]@{
        title = "Teste tecnico A21 R2 - $testMarker"
    ...
    if (-not $logout.ok) {
        throw 'LOGOUT A21 R2 NAO RETORNOU ok=true.'
    }
    ...
    if ($status2.authenticated -or -not $status2.token_configured -or $status2.write_enabled) {
        throw 'ESTADO FINAL A21 R2 INESPERADO APOS LOGOUT.'
    }
    Write-Log 'Logout: PASS; token permanece no Preview, mas escrita exige sessao valida.'

    Write-Log 'A21 R2 F1: PASS. Escrita e restauracao comprovadas exclusivamente em content/pa-v001-admin-preview.'
    Write-Host ''
    Write-Host 'PA SAFRA A21 R2 CONTENT WRITE LIVE: PASS' -ForegroundColor Green
    ...
    Write-Host ''
    Write-Host 'PA SAFRA A21 R2 CONTENT WRITE LIVE: FALHA' -ForegroundColor Red
    ...
    if ($MutationApplied -and -not $RestoreConfirmed) {
        Add-Content -LiteralPath $LogPath -Value '[ALERTA] O TESTE TEMPORARIO PODE PERMANECER NA BRANCH EDITORIAL; NAO AVANCAR ANTES DE AUDITAR/RESTAURAR.' -Encoding UTF8
    }
    exit 1
}
finally {
