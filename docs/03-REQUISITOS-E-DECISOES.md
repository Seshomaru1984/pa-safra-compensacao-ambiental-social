...
- **Situação:** CONFIRMADO; implementação existente; F1 FAIL no Preview por HTTP 500 no login válido.
...
- **Situação:** CONFIRMADO; D1/schema VERIFICADOS; comportamento live ainda pendente porque login não chegou ao gate completo.
...
|---|---|---|
| REQ-003 login/sessão | testes unitários `admin:auth-test`; Preview real | Unitário PASS; Preview FAIL HTTP 500 |
| REQ-004 rate limiter | D1 `admin_login_rate`; testes locais | Infra/schema PASS; live PENDENTE |
...
O defeito ativo é o HTTP 500 no login válido do Preview. O esperado continua sendo HTTP 200 com sessão válida. A implementação defeituosa não altera REQ-003. Diagnóstico deve separar Produto/Testador/Ambiente/Procedimento e usar a menor evidência capaz de diferenciar a causa sem repetir o testador `wrangler tail` que já falhou proceduralmente.