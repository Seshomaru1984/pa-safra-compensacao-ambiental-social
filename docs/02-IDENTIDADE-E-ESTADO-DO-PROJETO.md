...
| Escopo atual | P1 - integração do PR #39 em `develop`; `main` permanece fora do escopo autorizado |
...
- Reconstruir somente sobre `rebuild/pa-v001-clean` até a integração do PR #39.
...
- A integração ocorre por um único PR, `#39`, de `rebuild/pa-v001-clean` para `develop`.
- O usuário autorizou explicitamente em 14/09/2026 o merge do PR #39 em `develop`, com a exigência de respeitar as regras anexas. Essa autorização não se estende a `main` nem à publicação em produção.
...
| Fase atual | P1 - integração autorizada em `develop` |
| Base de integração verificada | `develop` em `e485f9270ef81680609d5fa2f1b9430ceaa4dbc0` antes do merge |
| Candidato autorizado | PR #39, branch `rebuild/pa-v001-clean` |
| HEAD funcional validado antes deste ajuste documental | `b5e4799d583226c1d3100d2fdbb0107ff18f5945` |
| CI do PR nesse HEAD | workflow `validate`, run `34901272170`, run number `1067`, SUCCESS |
| Branch em relação a `develop` antes deste ajuste documental | 317 commits à frente, 0 atrás |
| Mergeabilidade do PR antes deste ajuste documental | `mergeable=true` |
| Comentários/revisões pendentes | nenhum comentário e nenhuma revisão registrados |
...
| PR aberto da reconstrução | `#39` |
| Autorização para merge em `develop` | EXPLÍCITA em 14/09/2026 |
| Publicação em produção | NÃO AUTORIZADA |

Este ajuste documental altera o HEAD do candidato. O merge somente pode ocorrer depois de nova validação do HEAD final e nova confirmação de que a branch permanece 0 commits atrás de `develop` e o PR permanece mergeável.
...
- Run 1067: validação completa do PR no HEAD funcional `b5e4799d583226c1d3100d2fdbb0107ff18f5945` antes deste ajuste documental.
...
Executar o workflow `validate` no HEAD final após a atualização documental. Se ficar verde, reconfirmar que a branch está 0 commits atrás de `develop`, confirmar `mergeable=true` e então mesclar o PR #39 em `develop` usando o HEAD validado como precondição. Depois do merge, verificar o commit de integração e o workflow disparado em `develop`, atualizar este estado canônico e manter `main` bloqueada.